...
function makeRequestArray(){
	// START TO TEMP CODE
	// THIS DATA WILL BE IN THE PHP SESSION
	$wAPIKey = 'nufNixPhWpPxanqeLVJZvJaYKrdmXxuH';
	$wFirstName = 'Remi';
	$wLastName = 'GATTAZ';
	$wEmail = 'gattazr@students.wwu.edu';
	// END OF TEMP CODE

	// secret informations
	$wSecret = 'kwywZTiKpyzZlfVYWsaejTbwxKxTPWBq';
	$wTimestamp = gmdate("Y-m-d H:i:s", time());

	// make Hash
	$wToHash = $wFirstName.$wEmail.$wAPIKey.$wTimestamp.$wLastName;
	$wHash = hash_hmac('sha512', $wToHash, $wSecret);

	return array(
		'client' => 1, // number of the client to identify secret used
		'APIKey' => $wAPIKey, // key linked to a User and used to retrieve ingo on the API
		'timestamp' => $wTimestamp, // time at wich the request is made
		'hash' => $wHash // hash created using secret. Used to make sure request is from legit source
	);
